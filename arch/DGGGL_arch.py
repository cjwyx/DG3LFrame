...
class AGCRNCell_gatembedding(nn.Module):
    def __init__(self, node_num, dim_in, dim_out,gate_emb_dim, cheb_k):
        super(AGCRNCell_gatembedding, self).__init__()
        self.dim_in=dim_in
        self.dim_out=dim_out
        self.node_num = node_num
        self.hidden_dim = dim_out
        self.gate = AGCN(dim_in+self.hidden_dim+gate_emb_dim, 2*dim_out, cheb_k)
        self.update = AGCN(dim_in+self.hidden_dim, dim_out, cheb_k)

    def forward(self, x, state,gatembedding, supports):
        #x: B, num_nodes, input_dim
        #state: B, num_nodes, hidden_dim
        #gatembedding: B, num_nodes, embedding_dim
        state = state.to(x.device)
        input_and_state_with_gateembedding = torch.cat((x, state,gatembedding), dim=-1)
        z_r = torch.sigmoid(self.gate(input_and_state_with_gateembedding, supports))
        z, r = torch.split(z_r, self.hidden_dim, dim=-1)
        candidate = torch.cat((x, z*state), dim=-1)
        hc = torch.tanh(self.update(candidate, supports))
        h = r*state + (1-r)*hc
...
class AGCRNCell_gatembedding2(nn.Module):
    def __init__(self, node_num, dim_in,dim_state, dim_out,gate_emb_dim, cheb_k):
        super(AGCRNCell_gatembedding2, self).__init__()
        assert dim_state==dim_out, "statedim and outdim must same"
        self.dim_in=dim_in
        self.dim_out=dim_out
        self.node_num = node_num
        self.dim_state = dim_state
        # self.hidden_dim = dim_out
        self.gate = AGCN(dim_in+self.dim_state*2+gate_emb_dim, 2*self.dim_state+2*self.dim_out, cheb_k)
        self.update1 = AGCN(dim_in+self.dim_state+self.dim_state, dim_out, cheb_k)
        # self.update2 = AGCN(dim_in+self.dim_state, dim_out, cheb_k)
    def forward(self, x, state1,state2,gatembedding, supports):
        ...
        totual_state = torch.cat((state1,state2),dim=-1)
        
        
        input_and_state_with_gateembedding = torch.cat((x, totual_state,gatembedding), dim=-1)
        rs1 = torch.sigmoid(self.fcgate(input_and_state_with_gateembedding))
        state = rs1*state1 + (1-rs1)*state2
        
        
        
        z_r = torch.sigmoid(self.gate(torch.cat((x,state)), supports))
        z1z2,r1,r2 = torch.split(z_r,[self.d],dim=-1) 
        # z, r = torch.split(z_r, self.hidden_dim, dim=-1)
        candidate = torch.cat((x, z1z2*totual_state), dim=-1)
        hs = torch.tanh(self.update1(candidate, supports))
        hs1 = r1*state1 + (1-r1)*hs
        h = r2*state2 + (1-r2)*hs1
        return h
...
class AGCRNCell_gatembedding4(nn.Module):
    def __init__(self, node_num, dim_in,dim_state, dim_out,gate_emb_dim, cheb_k):
        super(AGCRNCell_gatembedding4, self).__init__()
        self.dim_in=dim_in
        self.dim_out=dim_out
        self.node_num = node_num
        self.state_dim = dim_state
...
class ADCRNN_Decoder2(nn.Module):
    def __init__(self, node_num, dim_in, dim_out,hiddenoutput_dim, cheb_k):
        '''
        
        '''
        super(ADCRNN_Decoder2, self).__init__()
        self.node_num = node_num
        self.input_dim = dim_in
        # self.num_layers = num_layers
        self.dcrnn_cell = AGCRNCell(node_num, dim_in, dim_out, cheb_k)
        self.hop_proj = nn.Linear(dim_out,hiddenoutput_dim,bias=False)

    def forward(self, xt, init_state, supports):
        # xt: (B, N, D)
        # init_state: (B, N, hidden_dim)
        assert xt.shape[1] == self.node_num and xt.shape[2] == self.input_dim
        current_state = self.dcrnn_cell(xt, init_state, supports)
        transnext_state = self.hop_proj(current_state)
        return current_state, transnext_state


class ADCRNN_Decoder3(nn.Module):
    def __init__(self, node_num, dim_in, dim_out,gate_emb_dim,hiddenoutput_dim, cheb_k):
        '''
        
        '''
        super(ADCRNN_Decoder3, self).__init__()
        self.node_num = node_num
        self.input_dim = dim_in
        # self.num_layers = num_layers
        self.dcrnn_cell = AGCRNCell_gatembedding(node_num, dim_in, dim_out,gate_emb_dim,cheb_k)
        self.hop_proj = nn.Linear(dim_out,hiddenoutput_dim,bias=False)

    def forward(self, xt, init_state,gatembedding, supports):
        # xt: (B, N, D)
        # init_state: (B, N, hidden_dim)
        assert xt.shape[1] == self.node_num and xt.shape[2] == self.input_dim
        current_state = self.dcrnn_cell(xt, init_state,gatembedding, supports)
        transnext_state = self.hop_proj(current_state)
        return current_state, transnext_state
...
class DGGGL(nn.Module):
    def __init__(self, num_nodes, input_dim, output_dim, horizon, rnn_units, gatoutput=32,num_layers=1, cheb_k=3,
        ...
        self.proj = nn.Sequential(
            nn.Linear(self.rnn_units, feed_forward_dim, bias=True),
            nn.GELU(),
            nn.Linear(feed_forward_dim, self.output_dim, bias=False),
